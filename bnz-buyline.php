	...
	function bnz_buyline_init() {
		...
		class BNZ_Buyline extends WC_Payment_Gateway {
			...
			public function __construct() {
				...
				$this->clientidhash = sanitize_text_field($this->settings['clientidhash']);
				$this->reliability_password = sanitize_text_field($this->settings['reliability_password']);
				$this->salt_value = sanitize_text_field($this->settings['salt_value']);
				$this->redirect_page_id = sanitize_text_field($this->settings['redirect_page_id']);
				$this->liveurl = 'https://www.buylineplus.co.nz/hosted/showEntry';
				
				$this->cid = $this->clientidhash;
				$this->rp = $this->reliability_password;
			...
			public function generate_bnz_buyline_form( $order_id ) {
				...
				$src = $this->liveurl . '?clientIdHash=' . $this->clientidhash . '&receiptMethod=CLOSEGET&metaData1=' . $order_id . '&paymentAmount=' . $order->order_total;
		...
		function bnz_buyline_response() {
			...
			if ( isset( $_REQUEST['metaData1'] ) && isset( $_REQUEST['reqid'] ) ) {
				...
				$resurl = "https://www.buylineplus.co.nz/hosted/processEntry/" . $cid . "/" . $rp . "/" . $reqid;
